...
func TestStdinManifestPath(t *testing.T) {
	expected := "-"
	if config.StdinManifestPath != expected {
		t.Errorf("StdinManifestPath = %q, want %q", config.StdinManifestPath, expected)
	}
}

func TestDefaultManifestPath(t *testing.T) {
	cmd := "testcmd"
	result := config.DefaultManifestPath(cmd)
	...
	expected := filepath.Join(config.Dir(cmd), "default")
...
func TestDefaultNamespace(t *testing.T) {
	result := config.DefaultNamespace()
...
func TestConfigIntegration(t *testing.T) {
	...
	dir := config.Dir(cmd)
	manifestPaths := config.DefaultManifestPath(cmd)
