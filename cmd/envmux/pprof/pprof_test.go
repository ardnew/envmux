...
func TestProfiler_Start(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with mode",
			prof: Profiler{Mode: "cpu", Path: "./tmp", Quiet: true},
		},
		{
			name: "mode with path delimiter",
			prof: Profiler{Mode: "cpu=./custom/path"},
		},
		{
			name: "quiet mode",
			prof: Profiler{Mode: "mem", Quiet: true},
		},
	}
...
func TestIgnoreType(t *testing.T) {
	...
	prof := Profiler{Mode: ""} // Empty mode uses ignore
