...
func TestWrap(t *testing.T) {
	...
	result = pkg.Wrap(initial, multiply2)
	expected := 20
	if result != expected {
	...
	result = pkg.Wrap(initial, add10, divide2)
	expected = 10 // (10 + 10) / 2
	if result != expected {
...
func TestWrapWithStruct(t *testing.T) {
	...
	initial := testStruct{}
	result := pkg.Wrap(initial, setName, setValue)
